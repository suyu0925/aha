...
/**
 * 本抽象类实现了使用Handle方法来异步处理消息 只要继承本类，并重写void processMessage(Message msg)方法即可
 * 
 * @author ccf
 */
public abstract class MyFeiGeBaseActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mNotification = new Notification(android.R.drawable.stat_sys_download, "飞鸽接收文件",
                System.currentTimeMillis());
        mNotification.contentView = new RemoteViews(getPackageName(),
                R.layout.file_download_notification);
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case IpMessageConst.IPMSG_SENDMSG | IpMessageConst.IPMSG_FILEATTACHOPT: {
                    ...
                    final String[] extraMsg = (String[]) msg.obj; // 得到附加文件信息,字符串数组，分别放了
                                                                  // IP，附加文件信息,发送者名称，包ID
                                    ...
                                    new DialogInterface.OnClickListener() {
                                        ...
                                        public void onClick(DialogInterface dialog, int which) {
                                            ...
                                            Thread fileReceiveThread = new Thread(
                                                    new NetTcpFileReceiveThread(extraMsg[3],
                                                            extraMsg[0], fileInfos)); // 新建一个接受文件线程
                                            fileReceiveThread.start(); // 启动线程

                                            Toast.makeText(getCurrentActivity(), "开始接收文件",
                            ...
                            .setNegativeButton("取消",
                                    new DialogInterface.OnClickListener() {
                                        public void onClick(DialogInterface dialog, int which) {
                                            ...
                                            netThreadHelper.sendUdpData(
                                                    ipmsgSend.getProtocolString(), sendAddress,
                                                    IpMessageConst.PORT);
                ...
                case UsedConst.FILERECEIVEINFO: { // 更新接收文件进度条
                    ...
                    queue.getLast().mNotification.contentView.setProgressBar(R.id.pd_download, 100,
                            sendedPer[1], false);
                    queue.getLast().mNotification.contentView.setTextViewText(R.id.fileRec_info,
                            "文件" + (sendedPer[0] + 1) + "接收中:" + sendedPer[1] + "%");
