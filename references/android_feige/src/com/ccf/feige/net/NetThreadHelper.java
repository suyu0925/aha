...
/**
 * 飞鸽的网络通信辅助类 实现UDP通信以及UDP端口监听 端口监听采用多线程方式 单例模式
 * 
 * @author ccf V1.0 2012/2/14，寂寞的情人节版本，嘿嘿
 */
        ...
        while (onWork) {
            ...
            switch (commandNo2) {
                case IpMessageConst.IPMSG_BR_ENTRY: { // 收到上线数据包，添加用户，并回送IPMSG_ANSENTRY应答。
                    ...
                    sendUdpData(ipmsgSend.getProtocolString(), udpResPacket.getAddress(),
                            udpResPacket.getPort()); // 发送数据
                }
                    break;
                ...
                case IpMessageConst.IPMSG_SENDMSG: { // 收到消息，处理
                    ...
                    if ((commandNo & IpMessageConst.IPMSG_FILEATTACHOPT) == IpMessageConst.IPMSG_FILEATTACHOPT) {
                        ...
                        String[] extraMsg = {
                                senderIp, splitStr[1], senderName, ipmsgPro.getPacketNo()
                        };
