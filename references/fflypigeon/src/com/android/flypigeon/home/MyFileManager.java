...
public class MyFileManager extends ListActivity {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        buttonCancle.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                /*
                 * if(selectType ==
                 * Constant.SELECT_FILE_PATH){//如果当前为选择文件夹模式，说明现在是保存文件操作
                 * ，如果这个cancel按钮被点击 Intent intent = new Intent();
                 * //说明用户拒绝接收文件，则向对方发送一个拒绝接收文件的指令。
    ...
    /**
     * @param filePath 需要打开的目录路径 打开该目录并获得里面的所有文件信息，包括目录和文件
    ...
    // =========================================================================================================================
    /*
     * private void openFile(File f) { Intent intent = new Intent();
     ...
     * startActivity(intent); } // 判断文件MimeType的method private String
     * getMIMEType(File f) { String type=""; String fName=f.getName(); // 取得扩展名
     * String end=fName.substring(fName.lastIndexOf(".")
     * +1,fName.length()).toLowerCase(); //依扩展名的类型决定MimeType
     ...
     * //如果无法直接打开，就跳出软件列表给用户选择 if(end.equals("apk")) { } else { type += "/*"; }
     * return type; }
     */
}
